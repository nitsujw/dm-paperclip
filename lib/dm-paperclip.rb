...
require 'dm-paperclip/jobs/delayed_paperclip_job'
require 'dm-paperclip/jobs/resque_paperclip_job'
...
module Paperclip
	...
	module ClassMethods
		...
		def process_in_background(name)
			...
			define_method "halt_processing_for_#{name}" do
				throw :halt unless self.send("#{name}_changed?")
			end

			define_method "enqueue_job_for_#{name}" do
				return if self.send("#{name}_changed?")

				if delayed_job?
					Delayed::Job.enqueue DelayedPaperclipJob.new(self.class.name, self.id, name.to_sym)
				elsif resque?
					Resque.enqueue(ResquePaperclipJob, self.class.name, self.id, name.to_sym)
				else
					raise "No job system"
				end
			end
			...
			define_method "#{name}_post_process" do
				attachment_for(name).post_process
			end

			before :"#{name}_post_process", :"halt_processing_for_#{name}"

			before :save, :"#{name}_processing!"
			after :save,  :"enqueue_job_for_#{name}"
		end
	...
	module InstanceMethods #:nodoc:
		...
		def column_exists?(column)
			self.class.properties.named?(column.to_s)
		end
